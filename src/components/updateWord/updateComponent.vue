...
<template>
  <v-dialog ref="dialog" v-model="modal" persistent width="1024px">
    <v-form ref="form" class="v-card theme--light" lazy-validation scrollable color="black">
      <v-card-title class="mt-2" label>Update Word</v-card-title>
  ...
  methods: {
    ...
    submitHandler: async function() {
      ...
      if (this.$v.$invalid) {
        ...
        axios
          .put(
            config.apiUrl + `updateWord/${this.wordJson.wordId}`,
            this.wordJson
          )
          .then(res => {
            console.log(res);
    ...
    updateWordList() {
      this.$bus.$emit("updateData", this.wordJson);
    }
  },
  async mounted() {
